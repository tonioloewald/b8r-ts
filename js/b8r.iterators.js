...
const mapKeys = (object, method) => {
    ...
    const map = [];
    for (const key of keys)
        map.push(method(object[key], key));
    return map;
};
const mapEachKey = (object, method) => {
    ...
    const map = {};
    for (const key of keys)
        map[key] = method(object[key], key);
    return map;
};
const findKey = (object, test) => {
    const keys = Object.keys(object);
    for (const key of keys)
        if (test(object[key], key))
            return key;
    return null;
};
...
export { makeArray, last, forEach, forEachKey, deepClone, mapKeys, mapEachKey, findKey, findValue, filterInPlace, filterKeys, filterObject, filterObjectInPlace, assignValues };
