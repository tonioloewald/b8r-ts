...
import { findWithin } from './b8r.dom';
import { get, call } from './b8r.registry';
import anyElement from './b8r.anyElement';
import * as keys from './b8r.keystroke';
import { pathSplit } from './b8r.byPath';
import implicitEventTypes from './b8r.implicit-event-types';
import { dispatch } from './b8r.dispatch';
...
function on(...args) {
    const config = onOffArgs(args);
    if (!config) {
        return;
    }
    const { element, eventType, path, prepend } = config;
...
if (window.TouchEvent) {
    ['touchstart', 'touchcancel', 'touchmove', 'touchend'].forEach(type => implicitEventTypes.push(type));
}
...
const handleEvent = (evt) => {
    ...
    if (evt.target.closest('[data-list],b8r-component:not([data-component-id]),[data-component]:not([data-component-id])'))
        return;
...
export { makeHandler, getEventHandlers, getParsedEventHandlers, dispatch, trigger, on, off, enable, disable, callMethod, implicitlyHandleEventsOfType, implicitEventTypes, getComponentWithMethod, handleEvent };
