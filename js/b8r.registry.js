...
/* global console */
import { getByPath, setByPath, deleteByPath } from './b8r.byPath';
import { getDataPath, getComponentId, splitPaths } from './b8r.bindings';
import { matchType } from './b8r.byExample';
import { componentTypes } from './b8r.component';
import { _b8r_ } from './b8r._b8r_';
import { observerShouldBeRemoved } from './b8r.constants';
...
const touch = (path, sourceElement = null) => {
...
export const offTypeError = (callback, restoreDefault = false) => {
    ...
    if (restoreDefault)
        onTypeError(_defaultTypeErrorHandler);
...
const checkType = (action, name) => {
    ...
    if (!referenceType || !registry[name])
        return;
...
const set = (path, value, sourceElement = null) => {
    if (debugPaths && !isValidPath(path)) {
...
const replace = (path, value) => {
    if (typeof value === 'object')
        setByPath(registry, path, null); // skip type checking
    set(path, value);
    return value;
};
...
const register = (name, obj, blockUpdates = false) => {
    if (name.match(/^_[^_]*_$/)) {
...
const _getByPath = (model, path) => get(path ? model + (path[0] === '[' ? path : '.' + path) : model);
export { get, getJSON, _getByPath as getByPath, set, replace, setJSON, increment, decrement, zero, push, unshift, sort, call, callIf, touch, observe, unobserve, models, _register, checkType, registerType, types, register, registered, remove, deregister, resolvePath, isValidPath };
