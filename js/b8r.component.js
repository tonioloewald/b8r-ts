...
*/
import { asyncUpdate } from './b8r.update';
import { create, find, findWithin } from './b8r.dom';
import { ajax } from './b8r.ajax';
import makeStylesheet from './b8r.makeStylesheet';
import uuid from './uuid';
import { AsyncFunction } from './b8r.functions';
const components = {};
...
const processComponent = (css, html, name) => {
    ...
    for (const elt of findWithin(view, '[class*="_component_"]')) {
        elt.setAttribute('class', elt.getAttribute('class').replace(/_component_/g, className));
    }
    return { style, view };
};
const makeComponentNoEval = function (name, { css, html, load, initialValue, type }) {
    const { style, view } = processComponent(css, html, name);
    const component = {
        version: 2,
        name,
        style,
...
const makeComponent = (...args) => {
    let [name, source, url, preserveSource] = args;
    ...
    const { style, view } = processComponent(css, content, name);
    /* jshint evil: true */
    let load = () => console.error('component', name, 'cannot load properly');
    // check for legacy components
    if (typeof script === 'string' && script.match(/[\b_]require\b/) && !script.match(/\belectron-require\b/)) {
    ...
    if (typeof script === 'string' && script.match(/\/\*\s*global[^*]*\bdata\b/)) {
    ...
    try {
        load = script
            ...
            ? new AsyncFunction('component', 'b8r', 'find', 'findOne', 'data', 'register', 'get', 'set', 'on', 'touch', `${script}\n//# sourceURL=${name}(component)`)
            : false;
    }
    catch (e) {
...
const component = (name, url, preserveSource = false) => {
    ...
    if (!componentPromises[name] || preserveSource) {
        if (!url)
            throw new Error(`expected component ${name} to be defined`);
        url = collapse(url);
...
export { component, components, componentTypes, componentTimeouts, componentPreloadMap, makeComponent, makeComponentNoEval };
