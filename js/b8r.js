...
import * as constants from './b8r.constants';
import { getByPath, pathSplit } from './b8r.byPath';
import * as _dom from './b8r.dom';
import * as _iterators from './b8r.iterators';
import * as _registry from './b8r.registry';
import * as _byExample from './b8r.byExample';
import * as _functions from './b8r.functions';
import _toTargets from './b8r.toTargets';
import * as _ajax from './b8r.ajax';
import * as _sort from './b8r.sort';
import { on, off, enable, disable, trigger, callMethod, implicitEventTypes, implicitlyHandleEventsOfType, handleEvent } from './b8r.events';
import { _insertSet, _insertFromTargets } from './b8r._b8r_';
import * as fromTargets from './b8r.fromTargets';
import { addDataBinding, removeDataBinding, getDataPath, getListPath, getListInstancePath, getComponentId, findLists, findBindables, getBindings, replaceInBindings, resolveListInstanceBindings, splitPaths } from './b8r.bindings';
import { saveDataForElement, dataForElement } from './b8r.dataForElement';
import { onAny, offAny, anyListeners } from './b8r.anyEvent';
import { keystroke, modifierKeys } from './b8r.keystroke';
import { asyncUpdate, getUpdateList, afterUpdate, touchElement, touchByPath, _afterUpdate, _setForceUpdate, expectCustomElement } from './b8r.update';
import { show, hide } from './b8r.show';
import { component, components, componentTimeouts, makeComponent, makeComponentNoEval } from './b8r.component';
...
let b8r;
const cleanupComponentInstances = _functions.debounce(() => {
...
const forceUpdate = () => {
    ...
    while (updateList = getUpdateList()) { // eslint-disable-line no-cond-assign
        ...
        let binds = null; // avoid collecting elements before big list updates
        ...
        if (binds)
            binds.forEach(({ elt, dirty }) => dirty && bind(elt));
    }
    b8r.cleanupComponentInstances();
    _afterUpdate();
};
const setByPath = function (...args) {
...
const pushByPath = function (...args) {
...
const unshiftByPath = function (...args) {
    let name, path, value;
    if (args.length === 2) {
        [path, value] = args;
        [name, path] = pathSplit(path);
...
const removeListInstance = function (elt) {
...
const removeByPath = function (...args) {
    let name, path, key;
    if (args.length === 2) {
        [path, key] = args;
        [name, path] = pathSplit(path);
...
const listItems = element => b8r.makeArray(element.children)
    .filter(elt => elt.matches('[data-list-instance]'));
const listIndex = element => b8r.listItems(element.parentElement).indexOf(element);
const getComponentData = (elt, type) => {
    const id = getComponentId(elt, type);
    return id ? b8r.get(id) : null;
};
const setComponentData = (elt, path, value) => {
...
const interpolate = (template, elt) => {
    let formatted;
    if (template.match(/\$\{[^{]*?\}/)) {
        formatted = template;
        do {
...
function bindList(listTemplate) {
    ...
    if (!listTemplate.parentElement || // skip if disembodied
        ...
        (() => {
            try {
                ...
                if (Array.isArray(list) &&
                    ...
                    console.warn(`list filter ${methodPath} returned a new object` +
    ...
    if (listContentChanged)
        b8r.trigger('change', listTemplate.parentElement);
    b8r.hide(listTemplate);
}
const bindAll = element => {
...
const _DEPRECATED_COMPONENTS_PASS_DOWN_DATA = false;
const inheritData = element => {
    ...
    if (!source) {
        ...
        return b8r.filterObject(data || {}, (v, k) => (!reserved.includes(k)) || typeof v !== 'function');
    }
};
let componentCount = 0;
const _componentInstances = {};
const insertComponent = async function (component, element, data) {
    if (!component) {
    ...
    if (typeof component === 'string') {
        if (!components[component]) {
            if (!componentTimeouts[component]) {
                ...
                componentTimeouts[component] = setTimeout(() => console.error('component timed out: ', component), 5000);
            }
            if (data) {
    ...
    */
    ...
    data = Object.assign(Object.assign(Object.assign({}, (component.type ? b8r.deepClone(component.type) : {})), initialValue), { dataPath,
        componentId });
    ...
    if (component.load) {
        try {
            await component.load(element, _pathRelativeB8r(component.path), find, findOne, data, register, get, set, on, touch, component);
        }
        catch (e) {
...
const Component = webComponents.makeWebComponent('b8r-component', {
    attributes: {
        name: '',
        path: '',
        value: null
...
const wrapWithComponent = (component, element, data, attributes) => {
...
const componentOnce = async (...args) => {
    const c = await b8r.component(...args);
    if (!b8r.findOne(`[data-component-id*="${c.name}"]`)) {
        await b8r.insertComponent(c);
    }
};
b8r = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ constants }, _dom), _iterators), _registry), _byExample), _functions), _ajax), _sort), { webComponents,
    on, off, enable, disable, trigger, callMethod, implicitlyHandleEventsOfType,
    onAny, offAny, anyListeners,
    addDataBinding, removeDataBinding, getDataPath, getComponentId, getListPath, getListInstancePath,
    keystroke, modifierKeys,
    asyncUpdate, afterUpdate, touchElement, touchByPath,
    component, makeComponent, makeComponentNoEval,
    ...
    wrapWithComponent,
    show,
    hide, components: () => Object.keys(components) });
_insertSet(b8r.set);
_insertFromTargets(fromTargets);
b8r.observe(() => true, (path, sourceElement) => {
    b8r.touchByPath(path, sourceElement);
});
