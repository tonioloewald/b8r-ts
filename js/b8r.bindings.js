...
*/
/* global console */
import { findWithin, succeeding } from './b8r.dom';
import { touchElement } from './b8r.update';
...
const parseBinding = binding => {
    ...
    const [, targetsRaw, path] = binding.trim().match(/^([^=]*)=([^;]*)$/m).map(s => s.trim());
    const targets = targetsRaw.split(',').map(target => {
        ...
        if (!parts)
            return null;
        if (parts[1].includes('.'))
            return { target: 'method', key: parts[1] };
...
const getComponentId = (element, type = false) => {
    if (type) {
...
export { addDataBinding, removeDataBinding, getDataPath, getListPath, getListInstancePath, getComponentId, parseBinding, findLists, findBindables, getBindings, replaceInBindings, resolveListInstanceBindings, splitPaths };
