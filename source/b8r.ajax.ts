...
const ajax = (url, method = 'GET', requestData: string|object = '', config: AjaxRequestConfig = {}) => {
...
const json = (url, method, requestData, config) => {
  return new Promise(function (resolve, reject) {
    ajax(url, method, requestData, config).then(data => {
      try {
        // @ts-ignore cannot understand tsc's objection to this
        resolve(JSON.parse(data || 'null'))
      } catch (e) {
        console.error('Failed to parse data', data, e)
        // @ts-ignore since the user may choose to handle more than one argument, tsc is WRONG
        reject(e, data)
      }
    }, reject)
  })
